...
Three automated workflows provide continuous integration and delivery for the entire ML lifecycle:
...
\midrule
\texttt{feature\_pipeline\_cron.yml} & Every hour & Ingest real-time data from AQICN and OpenWeather APIs, apply 37-feature engineering transformations, push versioned dataset to ClearML Feature Store \\
\texttt{training\_pipeline\_cron.yml} & Daily 2AM UTC & Extract latest training data, train all 9 models with cross-validation, compute SHAP/LIME explanations, evaluate metrics, promote champion model to ClearML registry, upload training artifacts \\
\texttt{infra\_validation.yml} & PR on \texttt{infrastructure/**} & Execute LangGraph multi-agent validation pipeline (Linter $\rightarrow$ Security $\rightarrow$ Policy $\rightarrow$ Decision) to gate infrastructure changes \\
\bottomrule
\end{tabularx}
\end{table}

\section{Infrastructure Validation (LangGraph Multi-Agent)}

A sophisticated multi-agent system powered by LangGraph validates all infrastructure changes before they are merged. This prevents misconfigured or insecure Terraform from reaching production.

\begin{enumerate}
    \item \textbf{Linter Agent:} Validates Terraform HCL syntax, checks formatting with \texttt{terraform fmt}, and enforces naming conventions and module structure best practices.
    \item \textbf{Security Agent:} Executes Checkov static analysis to identify security vulnerabilities including unencrypted storage, overly permissive IAM policies, and missing logging configurations.
    \item \textbf{Policy Agent:} Evaluates custom OPA/Rego organizational policies defined in \texttt{infrastructure/validation\_agents/policies/aws\_compliance.rego} covering resource tagging, encryption requirements, and cost controls.
    \item \textbf{Decision Agent:} Aggregates findings from all three agents, applies severity weighting, and issues a final pass/fail determination. On failure, generates actionable remediation suggestions.
\end{enumerate}

The validation pipeline runs automatically on any pull request that modifies files under the \texttt{infrastructure/} directory, providing immediate feedback to developers before code review.

\section{Test Suite}

The project includes a comprehensive pytest test suite covering all pipeline stages:
...
\midrule
\texttt{test\_api.py} & Flask endpoint correctness: health response structure, predict returns 72 hours, model selection works, historical data respects parameters & 6 \\
\texttt{test\_data\_pipeline.py} & Data ingestion (API mocking), async retry logic, feature engineering output shape and column presence, null handling & 8 \\
\texttt{test\_feature\_pipeline.py} & ClearML Dataset creation, Parquet write/read, feature store retrieval with time filtering & 5 \\
\texttt{test\_training\_pipeline.py} & Model training runs to completion, metrics are finite, champion selection logic, model serialization round-trip & 7 \\
\texttt{test\_validation.py} & LangGraph agent execution, Rego policy evaluation, end-to-end validation pipeline pass/fail & 4 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
Frontend (Next.js) & Vercel & React 19 + TypeScript + Tailwind CSS 4, automatic deploys on git push, global CDN, serverless functions \\
...
CI/CD & GitHub Actions & Hourly feature pipeline, daily training pipeline, PR validation \\
\bottomrule
\end{tabularx}
\end{table}
...
The Next.js 16 dashboard is deployed on Vercel with the following configuration:
...
\begin{itemize}
    ...
    \item \textbf{Environment Variable:} \texttt{NEXT\_PUBLIC\_API\_URL} pointing to Render backend
    \item \textbf{Features:} Framer Motion animations, Plotly charts, model selector, SHAP/LIME visualization pages
    \item \textbf{Performance:} Edge-cached static assets, ISR for dynamic pages, Lighthouse score $>$ 90
\end{itemize}
...
gunicorn deployment.api.main:app --bind 0.0.0.0:$PORT --workers 2 --timeout 120
\end{lstlisting}

\begin{itemize}
    \item \textbf{Runtime:} Python 3.11, auto-detected from \texttt{requirements.txt}
    \item \textbf{Build:} \texttt{pip install -r requirements.txt}
    \item \textbf{Health Check:} \texttt{GET /health} endpoint (30s interval)
...
For enterprise-scale deployment, Terraform configurations provision a fully serverless AWS architecture:

\begin{table}[H]
\centering
\caption{AWS Resources (Terraform-managed)}
\label{tab:aws}
\begin{tabularx}{\textwidth}{l l X}
\toprule
\textbf{Service} & \textbf{Purpose} & \textbf{Configuration} \\
\midrule
Lambda & Feature pipeline (hourly) & Container image, 512MB RAM, 5min timeout \\
Lambda & API serving (Function URL) & Container image, 1GB RAM, 30s timeout \\
ECS Fargate & Training pipeline (daily) & 2 vCPU, 4GB RAM, spot instances \\
ECR & Container registry & Lifecycle: retain last 10 images \\
EventBridge & Scheduling & Cron expressions for hourly/daily triggers \\
SSM Parameter Store & Secrets management & Encrypted API keys (SecureString) \\
CloudWatch & Observability & Log groups with 30-day retention, custom metrics \\
CodePipeline & CI/CD & Source (GitHub) $\rightarrow$ Build (CodeBuild) $\rightarrow$ Deploy \\
\bottomrule
\end{tabularx}
\end{table}

\begin{lstlisting}[language=bash, caption={Terraform provisioning}]
cd infrastructure/terraform
terraform init        # Initialize providers and backend
terraform plan        # Preview resource changes
terraform apply       # Provision infrastructure
terraform output      # Display endpoint URLs and ARNs
\end{lstlisting}
...
# 7. Launch Streamlit dashboard (separate terminal)
streamlit run deployment/streamlit_app/app.py

# 8. Launch Next.js frontend (separate terminal)
cd deployment/web_app && npm install && npm run dev

# 9. Run tests
pytest tests/ -v --cov=. --cov-report=term-missing

# 10. Docker deployment (alternative to steps 6-8)
...
Model Development & 9 trained models spanning 4 paradigms with Optuna Bayesian HPO (50 trials) \\
...
Automation & Hourly ingestion + daily retraining via GitHub Actions; LangGraph infra validation \\
Deployment & Next.js frontend (Vercel) + Flask API (Render) + Docker + Terraform IaC \\
Testing & 30+ pytest tests across 5 modules covering all pipeline stages \\
Documentation & 40+ page LaTeX technical report + README with Mermaid architecture diagrams \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    \item Achieved \textbf{R\textsuperscript{2} = 0.903} (champion model) on 72-hour ahead AQI forecasting with MAPE of 9.23\%
    ...
    \item Deployed a \textbf{production-ready system} with interactive Next.js dashboard, REST API, and automated CI/CD
\end{itemize}
