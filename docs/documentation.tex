...
\begin{tikzpicture}[remember picture, overlay]
    % Background stripes
    \fill[darkblue] (current page.north west) rectangle ([yshift=-6.5cm]current page.north east);
    \fill[primaryblue] ([yshift=-6.5cm]current page.north west) rectangle ([yshift=-6.7cm]current page.north east);
    \fill[accentgreen] ([yshift=-6.7cm]current page.north west) rectangle ([yshift=-6.8cm]current page.north east);
    
    % Abstract nodes for tech feel
    \foreach \i in {1,...,20} {
        \fill[white, opacity=0.15] ([xshift=rand*10cm, yshift=-rand*3cm-1cm]current page.north) circle (rand*0.2cm+0.1cm);
    }
    
    % Top Text Node tightly bound to the top
    \node[anchor=north, text centered, text=white] at ([yshift=-1.8cm]current page.north) {
        \begin{tabular}{c}
            {\Huge\bfseries\sffamily Air Quality Index Predictor} \\[12pt]
            {\Large\sffamily End-to-End Air Quality Forecasting System} \\[12pt]
            {\large\sffamily\color{primaryblue!20} Project Documentation}
        \end{tabular}
    };
\end{tikzpicture}
...
% Central decorative element (AQI Rings)
\begin{tikzpicture}
    % Outer glow rings
...
\begin{tabularx}{0.95\linewidth}{rX}
    \textcolor{darkblue}{\textbf{Target Domain:}} & Sargodha, Pakistan (32.08\textdegree N, 72.67\textdegree E) \\
    \textcolor{darkblue}{\textbf{Horizon:}} & 72-Hour (3-Day) Ahead \\
    \textcolor{darkblue}{\textbf{Architecture:}} & 9 ML Models (Linear $\rightarrow$ Sequence) \\
    \textcolor{darkblue}{\textbf{Stack:}} & Python, Flask, Next.js, PyTorch, ClearML \\
    \textcolor{darkblue}{\textbf{Deployment:}} & \href{https://aqi-predictor-3cawg37a4-giki.vercel.app}{\textbf{Frontend (Vercel)}} \ \textbar \ \href{https://pearls-aqi-api.onrender.com}{\textbf{API (Render)}} \\
    \textcolor{darkblue}{\textbf{Explainability:}} & SHAP + LIME + Temporal Grad-CAM \\
\end{tabularx}
\end{tcolorbox}
...
\begin{tikzpicture}[remember picture, overlay]
    % Background stripes for footer
    \fill[darkblue] (current page.south west) rectangle ([yshift=4.5cm]current page.south east);
    \fill[primaryblue] ([yshift=4.5cm]current page.south west) rectangle ([yshift=4.7cm]current page.south east);
    \fill[accentgreen] ([yshift=4.7cm]current page.south west) rectangle ([yshift=4.8cm]current page.south east);
    
    % Author Text properly anchored inside the footer
    \node[anchor=south, text centered, text=white] at ([yshift=1.2cm]current page.south) {
        \begin{tabular}{c}
            {\huge\bfseries\sffamily Zain Ul Abdeen} \\[8pt]
            {\Large\sffamily\color{accentgreen} zaniulabdeen9909@gmail.com} \\[12pt]
            {\normalsize\sffamily Machine Learning Engineer \ \textbar \ \ 10p Shine Internship Program \ \textbar \ \ August 2026}
        \end{tabular}
    };
\end{tikzpicture}
...
% ═══════════════════════════════════════════════════════════════════════════════
\subsection{Technology Deviations and Justifications}
...
\begin{tikzpicture}[
    ...
    % Serving
    \node[deploy, right=3cm of registry] (flask) {Flask REST API\\(9 endpoints + /v1/)};
    \node[deploy, above=of flask] (nextjs) {Next.js Dashboard\\(11 React components)};
...
\subsubsection{Feature Pipeline (Hourly)}
\begin{itemize}
...
\subsubsection{Training Pipeline (Daily)}
\begin{itemize}
...
\begin{itemize}
    \item \textbf{API:} Flask REST with 9 endpoints (predict, explain, explain/lime, health, historical, models, simulate, satellite, shadow) served at both root and \texttt{/v1/} prefix
    \item \textbf{Security:} API key authentication (opt-in), rate limiting (60 req/min per IP), request ID tracing
    \item \textbf{Frontend:} Next.js dashboard with 11 interactive React components including ErrorBoundary for fault isolation
...
\subsection{Historical Data Backfill}
...
\subsection{Pollutant Distributions}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{hour_month_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{monthly_aqi_timeseries.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{acf_pacf_analysis.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{hourly_acf_168h.png}
...
\subsection{Weather-Pollutant Relationships}
...
\subsection{Pollution Wind Rose}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{pollution_wind_rose.png}
\caption{Pollution by Wind Direction (16-Sector Analysis)}
\label{fig:windrose}
\end{figure}

\subsection{Granger Causality and Hypothesis Testing}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{granger_causality.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{feature_importance_mutual_info.png}
\caption{Top 20 Features by Mutual Information with AQI}
\label{fig:feature_importance}
\end{figure}

\subsection{Pair Plot and Outlier Analysis}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{pair_plot_by_aqi_level.png}
...
% ═══════════════════════════════════════════════════════════════════════════════
\section{CI/CD, Testing, and Automation}
% ═══════════════════════════════════════════════════════════════════════════════
...
\subsection{GitHub Secrets and Variables}
...
\midrule
\texttt{test\_api.py} & 31 Flask endpoint tests: health, predict, models, explain, simulate, historical, satellite, shadow, error handling \\
\texttt{test\_data\_pipeline.py} & 14 tests: synthetic data generation, feature engineering, temporal encodings, wind decomposition, lag features \\
\texttt{test\_e2e\_pipeline.py} & 8 end-to-end integration tests: ingest $\rightarrow$ transform $\rightarrow$ train $\rightarrow$ registry $\rightarrow$ predict $\rightarrow$ drift $\rightarrow$ anomaly \\
\texttt{test\_feature\_pipeline.py} & ClearML Dataset creation, feature store retrieval \\
\texttt{test\_training\_pipeline.py} & Model training completion, metrics are finite, evaluation pipeline \\
\texttt{load\_test.py} & Locust load test configuration (50 concurrent users, weighted endpoints) \\
\bottomrule
\end{tabularx}
\end{table}
...
\vspace{0.2cm}
\begin{center}
\textbf{Live Backend API:} \href{https://pearls-aqi-api.onrender.com}{\texttt{https://pearls-aqi-api.onrender.com}}
\end{center}
\vspace{0.2cm}
...
\vspace{0.2cm}
\begin{center}
\textbf{Live Frontend Application:} \href{https://aqi-predictor-3cawg37a4-giki.vercel.app}{\texttt{https://aqi-predictor-3cawg37a4-giki.vercel.app}}
\end{center}
\vspace{0.2cm}
...
\begin{itemize}
    ...
    \item \textbf{ErrorBoundary:} React class component wrapping all pages for graceful error recovery
    \item \textbf{12 Test Files:} Vitest + React Testing Library covering all components
\end{itemize}
...
The Flask backend exposes 9 endpoints, each available at both root path and \texttt{/v1/} prefix. Authentication via \texttt{X-API-Key} header is optional (enabled when \texttt{API\_AUTH\_KEY} env var is set). Rate limiting: 60 requests/minute per IP.
...
\texttt{POST} & \texttt{/predict} & 72-hour AQI forecast with RMSE-based confidence intervals \\
\texttt{POST} & \texttt{/explain} & SHAP feature contributions \\
\texttt{POST/GET} & \texttt{/explain/lime} & LIME local surrogate explanations \\
...
Frontend & Next.js 16 dashboard with 11 interactive React components, ErrorBoundary, model zoo selector, causal simulator, and particle engine \\
Deployment & Backend on Render + Frontend on Vercel + Docker for local/self-hosted use \\
Testing & pytest suite (70+ tests across 6 modules) + 12 frontend Vitest tests + Locust load tests \\
Documentation & LaTeX technical report + OpenAPI 3.1 spec + CONTRIBUTING.md + README with architecture diagrams \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    ...
    \item Deployed a \textbf{production-ready system} with an interactive Next.js dashboard, REST API with 9 endpoints (+ /v1/ versioning), API authentication, rate limiting, request tracing, Sentry error monitoring, and 6 automated CI/CD workflows
    \item Achieved \textbf{100\% frontend component test coverage} with 12 Vitest test files and 70+ backend pytest tests including end-to-end pipeline integration tests
\end{itemize}
...
\begin{itemize}
    ...
    \item Current confidence intervals use model RMSE scaled by $\sqrt{1+h/12}$ with proper z-scores (1.28 for 80\%, 1.96 for 95\%); formal conformal prediction would provide distribution-free theoretical guarantees
...
\texttt{API\_AUTH\_KEY} & -- & API authentication key (empty = disabled) \\
\texttt{SENTRY\_DSN} & -- & Sentry error tracking DSN \\
\texttt{CORS\_ORIGINS} & Vercel domains & Comma-separated allowed origins \\
\texttt{SERVER\_HOST} & 0.0.0.0 & API bind host \\
\texttt{SERVER\_PORT} & 8000 & API bind port \\
\bottomrule
\end{longtable}
...
|   |   +-- ensemble_trees.py  GradientBoosting / RF / ExtraTrees / SVR
|   |   +-- tree_ensemble.py   LightGBM + Optuna
|   |   +-- xgboost_model.py   XGBoost + Optuna
|   |   +-- deep_learning.py   Bi-LSTM + Multi-Head Attention
|   |   +-- callbacks.py       EarlyStopping, Checkpoints
|   |   +-- grad_cam.py        Temporal Grad-CAM
|   +-- registry.py             Multi-model versioning + champion promotion
+-- deployment/
|   +-- api/                    Flask REST API (9 endpoints + /v1/ versioning)
|   +-- web_app/                Next.js 16 dashboard (11 React components)
|   +-- streamlit_app/          Streamlit dashboard (alternative UI)
+-- tests/                      pytest suite (70+ tests, 6 modules)
