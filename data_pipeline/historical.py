...
class RealHistoricalDataFetcher:
    ...
    def fetch_all(self, start_date: str, end_date: str) -> list[RawDataPayload]:
        ...
        for i in range(len(times)):
            dt = datetime.fromisoformat(times[i]).replace(tzinfo=UTC)
            ...
            no2 = (
                aq_data["nitrogen_dioxide"][i]
                if aq_data["nitrogen_dioxide"][i] is not None
                else 0.0
            )
            so2 = (
                aq_data["sulphur_dioxide"][i] if aq_data["sulphur_dioxide"][i] is not None else 0.0
            )
            ...
            hum = (
                w_data["relative_humidity_2m"][i]
                if w_data["relative_humidity_2m"][i] is not None
                else 0.0
            )
            ws = w_data["wind_speed_10m"][i] if w_data["wind_speed_10m"][i] is not None else 0.0
            wd = (
                w_data["wind_direction_10m"][i]
                if w_data["wind_direction_10m"][i] is not None
                else 0.0
            )
            pres = (
                w_data["surface_pressure"][i]
                if w_data["surface_pressure"][i] is not None
                else 1013.25
            )
            ...
            pollutants = [
                PollutantReading(
                    name="pm25", value=max(0.0, float(pm25)), unit="µg/m³", timestamp=dt
                ),
                PollutantReading(
                    name="pm10", value=max(0.0, float(pm10)), unit="µg/m³", timestamp=dt
                ),
                PollutantReading(
                    name="no2", value=max(0.0, float(no2)), unit="µg/m³", timestamp=dt
                ),
                PollutantReading(
                    name="so2", value=max(0.0, float(so2)), unit="µg/m³", timestamp=dt
                ),
