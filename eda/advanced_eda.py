...
def autocorrelation_analysis(df: pd.DataFrame):
    ...
    acf_report = pd.DataFrame(
        {"lag": range(len(acf_values)), "acf": acf_values, "pacf": pacf_values}
    )
    ...
    ax.set_ylabel("ACF")
    ax.set_title(
        "Hourly Autocorrelation (168h = 7 days) - Strong 24h Periodicity", fontweight="bold"
    )
...
def hypothesis_testing(df: pd.DataFrame):
    ...
    results["winter_vs_summer"] = {
        ...
        "interpretation": "Winter AQI significantly higher"
        if p_val < 0.05
        else "No significant seasonal difference",
    }
    ...
    results["seasonal_kruskal_wallis"] = {
        ...
        "significant": bool(p_val < 0.05),
        "interpretation": "Significant seasonal differences exist"
        if p_val < 0.05
        else "No seasonal differences",
    }
    ...
    df_t["is_night"] = df_t["timestamp"].dt.hour.isin(range(0, 6)) | df_t["timestamp"].dt.hour.isin(
        range(20, 24)
    )
    ...
    results["day_night_variance"] = {
        ...
        "significant": bool(p_val < 0.05),
        "interpretation": "Unequal variance (heteroscedastic)"
        if p_val < 0.05
        else "Equal variance (homoscedastic)",
    }
    ...
    plt.close()
    print(
        f"  5 hypothesis tests completed. Seasonal diff: {results['seasonal_kruskal_wallis']['significant']}"
    )
...
def hourly_monthly_heatmap(df: pd.DataFrame):
    ...
    ax.set_title(
        "AQI Heatmap: Hour of Day vs Month\n(Sargodha, Pakistan - 5 Year Average)",
        fontsize=13,
        fontweight="bold",
    )
...
def rolling_volatility(df: pd.DataFrame):
    ...
    axes[1].axhline(
        vol_7.mean(), color="black", ls="--", alpha=0.5, label=f"Mean={vol_7.mean():.1f}"
    )
    axes[1].legend()
    ...
    axes[2].axhline(
        vol_30.mean(), color="black", ls="--", alpha=0.5, label=f"Mean={vol_30.mean():.1f}"
    )
    axes[2].legend()
    ...
    vol_stats = {
        ...
        "high_volatility_periods_pct": round(
            float((vol_7 > vol_7.quantile(0.9)).sum() / len(vol_7) * 100), 1
        ),
    }
...
def cross_pollutant_lag_correlation(df: pd.DataFrame):
    ...
    ax.set_title(
        "Lag-Correlation: How Past Pollutant Values Predict Current AQI",
        fontsize=13,
        fontweight="bold",
    )
...
def pollution_wind_analysis(df: pd.DataFrame):
    ...
    df_w["sector"] = ((df_w["wind_direction_deg"] + sector_width / 2) % 360 // sector_width).astype(
        int
    )
...
def granger_causality_test(df: pd.DataFrame):
    ...
    daily = (
        df.set_index("timestamp")[
            ["aqi_value", "temperature_c", "humidity_pct", "wind_speed_ms", "pressure_hpa"]
        ]
        .resample("D")
        .mean()
        .dropna()
    ...
    for col in ["temperature_c", "humidity_pct", "wind_speed_ms", "pressure_hpa"]:
        try:
            ...
            results[col] = {
                ...
                "all_p_values": {
                    f"lag_{i + 1}": round(float(p), 4) for i, p in enumerate(p_values)
                },
                "interpretation": f"{col} Granger-causes AQI at lag {best_lag}"
    ...
    ax.set_title(
        "Granger Causality: Weather Variables -> AQI\n(Green = Significant, Red = Not Significant)",
        fontweight="bold",
    )
    ax.legend()
    plt.tight_layout()
    ...
    plt.close()
    print(
        f"  Granger tests complete. Significant predictors: {[v for v, r in results.items() if r.get('significant')]}"
    )
...
def pair_plot_analysis(df: pd.DataFrame):
    ...
    g.figure.suptitle(
        "Pair Plot: Key Variables Colored by AQI Category", y=1.02, fontsize=13, fontweight="bold"
    )
